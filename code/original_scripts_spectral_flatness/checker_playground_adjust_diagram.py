...
def run():    
    ...
    files = ['Sat1-h22.mp3']# take only non-folders
    ...
    for file_num, f in enumerate(files):
        ...
        for (j, step) in enumerate(xrange(0, max_size, stepsize)):
            ...
            if show_figures:
                ...
                print_annotations(axarr[0], anno, len_per_hour, max_size, j*len(sflatness), (j+1)*len(sflatness), y_axis_max_size)
                ...
                #second subplot
                axarr[1].plot(sflatness_masked, alpha=0.7, linewidth=2.5)
               ...
               #third subplot PROTOTYP only for whole hours
                axarr[2].plot(sflatness_area, alpha=0.7, linewidth=2.5)
                print_annotations(axarr[2], anno, len_per_hour, max_size, j*len(sflatness), (j+1)*len(sflatness), y_axis_max_size)	    
                axarr[2].fill_between(range(sflatness_area.shape[0]), 0, sflatness_area, facecolor='blue', alpha=0.3)
