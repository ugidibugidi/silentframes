...
def get_threshold(channel, daytime):
    ...
    top_n_largest = (np.maximum(top_n_min, top_n_largest*factor)).astype(int)    
...
def run():    
    ...
    for file_num, f in enumerate(files):
        ...
        for (j, step) in enumerate(xrange(0, max_size, stepsize)):
            ...
            out_f = open('/home/user/Desktop/masterarbeit/data/betweenSilentFrames/' + f.replace(".mp3", ".betweenSilentFrames_heuristic2") , 'wb')
